...
class ChatApi {
  ...
  static async fetchMessages(groupId: string, limit: number): Promise<ChatMessage[]> {
    try {
      const response: AxiosResponse<ChatMessage[]> = await axiosInstance.get(`${this.API_BASE_URL}/messages`, {
  ...
  static async sendGroupMessage(groupId: string, message: string): Promise<void> {
    try {
      await axiosInstance.post(`${this.API_BASE_URL}/messages`, { groupId, message });
    } catch (error) {
  ...
  static async createGroup(groupName: string, isPublic: boolean): Promise<Group> {
    try {
      const response: AxiosResponse<Group> = await axiosInstance.post(`${this.API_BASE_URL}/groups`, {
        name: groupName,
        isPublic,
      });
      return response.data;
  ...
  static async toggleGroupPrivacy(groupId: string): Promise<void> {
    try {
      await axiosInstance.patch(`${this.API_BASE_URL}/groups/${groupId}/toggle-privacy`);
    } catch (error) {
