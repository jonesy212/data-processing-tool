...
const idleTimeoutCondition: IdleTimeoutConditionFunction = (
  ...
  return (
    ...
    Date.now() - lastUserInteractionTime >= IDLE_TIMEOUT_DURATION
  );
};

const showModalOrNotification: ShowModalOrNotificationFunction = (
  title,
  message
) => {
...
const fetchLastUserInteractionTime: FetchLastUserInteractionTimeFunction =
  async () => {
    try {
...
const resetIdleTimeout: ResetIdleTimeoutFunction = () => {
  const props: any = {};
  let idleTimeout: ReturnType<typeof useIdleTimeout> = useIdleTimeout(props);
  // Clear the existing idle timeout (if any)
  if (idleTimeout.idleTimeoutId) {
    clearTimeout(idleTimeout.idleTimeoutId);
  }

  // Start a new idle timeout
  idleTimeout.startIdleTimeout(IDLE_TIMEOUT_DURATION, onTimeout);
};
