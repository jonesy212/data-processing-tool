...
function flatMap<T extends  BaseData<T>, K extends T = T, Meta extends StructuredMetadata<T, K> = StructuredMetadata<T, K>>(
...
const createSnapshotInstance =  async <
  ...
  K extends T = T>(
    ...
    const baseSnapshot = await createBaseSnapshot(baseData, baseMeta, storeProps);
    ...
    const equalsFunction = (data: Snapshot<T, K>): boolean | null | undefined => {
      if (!data || !this.id) return null;
      return this.id === data.id && JSON.stringify(this.data) === JSON.stringify(data.data);
    };
    ...
    const snapshotWithSubscription = {
          ...
          try {
            ...
            if (!snapshotStoreConfig) {
              ...
              const newSnapshot: Snapshot<T, K> = {
                ...
                determinePrefix: function <T extends  BaseData<T>>(snapshot: T | null | undefined, category: string): string {
                  throw new Error("Function not implemented.");
                },
                ...
                batchFetchSnapshots: function (subscribers: Subscriber<T, K>[], snapshots: Snapshots<T>): Promise<Snapshots<T>> {
                  throw new Error("Function not implemented.");
                },
                ...
                label: undefined,
                events: undefined,
                handleSnapshot: function (id: string, snapshotId: string, snapshot: BaseData | null, snapshotData: BaseData, category: symbol | string | Category | undefined, callback: (snapshot: BaseData) => void, snapshots: Snapshots<T>, type: string, event: Event, snapshotContainer?: BaseData | undefined, snapshotStoreConfig?: SnapshotStoreConfig<BaseData, BaseData> | undefined): Promise<Snapshot<BaseData, BaseData> | null> {
                  throw new Error("Function not implemented.");
                }
              };
              ...
              if (existingSnapshot) {
                ...
                const newSnapshot: Snapshot<BaseData, BaseData> = {
                  ...
                  createSnapshotSuccess: function (snapshotId: string | number, snapshotManager: SnapshotManager<BaseData, BaseData>, snapshot: Snapshot<BaseData, BaseData>, payload: { error: Error; }): void | null {
                    throw new Error("Function not implemented.");
                  },
                  ...
                  onSnapshots: function (snapshotId: string, snapshots: SnapshotsArray<T>, type: string, event: Event, callback: (snapshots: Snapshots<T>) => void): void {
                    throw new Error("Function not implemented.");
                  },
                  label: undefined,
                  events: undefined,
                  handleSnapshot: function (
                    ...
                    category: symbol | string | Category | undefined,
                    callback: (snapshot: BaseData) => void, snapshots: Snapshots<T>,
                    type: string, event: Event,
      ...
      updateSnapshot: (
        snapshotId: string,
        data: Map<string, Snapshot<T, K>>,
        snapshotManager: SnapshotManager<T, K, undefined>,
      ...
      handleSnapshot: (
        ...
        switch (type) {
          case 'create': {
            ...
            const newSnapshot: SnapshotUnion<T> = {
              id: snapshotId,
              data: snapshotData,
              category: category,
              timestamp: new Date(),
    ...
    removeChild: (childId: string, parentId: string, parentSnapshot: CoreSnapshot<T, K>, childSnapshot: CoreSnapshot<T, K>) => {
      ...
      notifySubscribers: async (): Promise<Subscriber<T, K>[]> => {
        ...
        const snapshots = await snapshotStore.getAllSnapshots(
         ...
         // Convert the array of Snapshot<T, K> to Snapshots<T>
        const snapshotsArray: SnapshotsArray<T> = snapshots.map((snapshot: Snapshot<T, K>) => snapshot as SnapshotUnion<T>);
      ...
      getSnapshotsBySubscriberSuccess: (snapshots: Snapshots<T>) => {
