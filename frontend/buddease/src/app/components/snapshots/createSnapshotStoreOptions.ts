...
// import { convertToSubscriberCollection } from '@/app/components/utils/SubscriberUtils'
...
// import { getSubscription } from '@/app/components/subscriptions/subscriptionService'
...
// import { DataStoreWithSnapshotMethods, DataStoreMethods } from "../projects/DataAnalysisPhase/DataProcessing/ DataStoreMethods";
...
// const createSnapshotStoreOptions =  <T extends  BaseData<T>, K extends T = T, Meta extends StructuredMetadata<T, K> = StructuredMetadata<T, K>>({
//   initialState,
//   snapshotId,
//   category,
//   dataStoreMethods,
...
//           const processedSnapshots = await dataCallback(foundSubscribers, filteredSnapshots as unknown as Snapshots<T>);
...
//     batchFetchSnapshotsSuccess: (subscribers: SubscriberCollection<T, K>[], snapshots: Snapshots<T>) => {
...
//     },
//     batchUpdateSnapshotsSuccess: (subscribers: SubscriberCollection<T, K>, snapshots: Snapshots<T>) => {
...
//     },
//     createSnapshotSuccess: (snapshotId: string | number, snapshotManager: SnapshotManager<T, K, undefined>, snapshot: Snapshot<T, K>, payload?: { data?: any; } | undefined) => {
...
// const isSnapshotStoreOptions =  <T extends  BaseData<T>, K extends T = T, Meta extends StructuredMetadata<T, K> = StructuredMetadata<T, K>>(obj: any): obj is SnapshotStoreOptions<T, K> => {  
//   return obj && typeof obj === 'object' && 'data' in obj && 'initialState' in obj;
// };

// const getCurrentSnapshotStoreOptions =  <T extends  BaseData<T>, K extends T = T, Meta extends StructuredMetadata<T, K> = StructuredMetadata<T, K>>(
...
// const convertToArray =  <T extends  BaseData<T>, K extends T = T, Meta extends StructuredMetadata<T, K> = StructuredMetadata<T, K>>(
//   snapshotStore: SnapshotStore<T, K>,
...
// const handleSingleSnapshot = <T extends  BaseData<T>, K extends T = T, Meta extends StructuredMetadata<T, K> = StructuredMetadata<T, K>>(
//   snapshot: Snapshot<T, K>,
...
// const handleSnapshotsArray =  <T extends  BaseData<T>, K extends T = T, Meta extends StructuredMetadata<T, K> = StructuredMetadata<T, K>>(
//   snapshots: SnapshotsArray<T>,
//   callback: Callback<Snapshot<T, K>>
// ) => {
...
// const isSnapshotArrayState = <T extends  BaseData<T>, K extends T = T, Meta extends StructuredMetadata<T, K> = StructuredMetadata<T, K>>(state: any): state is Snapshot<T, K>[] => {
//   // Logic to determine if it's a Snapshot array
//   return Array.isArray(state) && state.every((item: any) => isSnapshot(item));
// };


// function toSnapshotsArray<T extends  BaseData<T>>(snapshots: Snapshot<T, any>[]): SnapshotsArray<T> {
//   // Transform if needed, or validate each snapshot type
//   return snapshots as SnapshotsArray<T>;
// }


// function isCompatibleSnapshot <T extends  BaseData<T>, K extends T = T, Meta extends StructuredMetadata<T, K> = StructuredMetadata<T, K>>(
//   snapshot: Snapshot<T, K>
...
// function isSnapshotUnion<T extends  BaseData<T>, K extends T>(
//   obj: any
...
//     toSnapshotsArray, convertSnapshotsObjectToArray, createSnapshotStoreOptions
// };