...
export const BatchMethods = {
  ...
  >(
  ...
  ): Promise<Snapshot<T, K, Meta, ExcludedFields>[] | { snapshots: Snapshots<T, K, Meta, ExcludedFields> }> {
    ...
    if (Array.isArray(itemsOrSnapshotId) && snapshotManagerOrStore instanceof SnapshotManager) {
      ...
      const takenSnapshots: Snapshot<T, K, Meta, ExcludedFields>[] = [];
  ...
  >(
    this: SnapshotStore<T, K, Meta, ExcludedFields>,
    updates: Array<{ id: string; newData: Partial<Snapshot<T, K, Meta, ExcludedFields>> }>
  ): void {
  ...
  >(
    ...
    if (ids && ids.length > 0) {
      const fetched: Snapshot<T, K, Meta, ExcludedFields>[] = [];
      for (const id of ids) {
  ...
  batchUpdateSnapshotsFailure(
    ...
    snapshotId: string, 
    snapshotManager: SnapshotManager<T, K, Meta, ExcludedFields>, 
    snapshot: Snapshot<T, K, Meta, ExcludedFields>, payload: { error: Error; }
