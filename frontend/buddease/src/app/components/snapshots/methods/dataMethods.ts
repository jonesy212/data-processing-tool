...
>(this: SnapshotStore<T, K, Meta, ExcludedFields>, data: Snapshot<T, K, Meta, ExcludedFields>): void {
...
>(
  ...
  const snapshot = snapshotStore.getSnapshot(id); // Assuming this returns Snapshot<T, K, Meta, ExcludedFields>
  ...
  return Promise.reject(
    new Error(`Snapshot with id ${id} is not of type Snapshot<T, K, Meta, ExcludedFields>`)
  );
}
...
export function updateData<
  ...
  Meta extends DefaultMeta<T, K> = DefaultMeta<T, K>,
  ExcludedFields extends keyof T = DefaultExcludedFields<T>
>(this: SnapshotStore<T, K, Meta, ExcludedFields>, id: number, newData: Snapshot<T, K, Meta, ExcludedFields>): void {
...
export function addDataSuccess<
  ...
  Meta extends DefaultMeta<T, K> = DefaultMeta<T, K>,
  ExcludedFields extends keyof T = DefaultExcludedFields<T>
>(this: SnapshotStore<T, K, Meta, ExcludedFields>, payload: { data: Snapshot<T, K, Meta, ExcludedFields>[] }): void {
...
export function setData<
  ...
  Meta extends DefaultMeta<T, K> = DefaultMeta<T, K>,
  ExcludedFields extends keyof T = DefaultExcludedFields<T>
>(this: SnapshotStore<T, K, Meta, ExcludedFields>, id: string, data: Map<string, Snapshot<T, K, Meta, ExcludedFields>>): void {
  try {
    if (!this.dataStore) {
...
export function addDataPartial<
  ...
  Meta extends DefaultMeta<T, K> = DefaultMeta<T, K>,
  ExcludedFields extends keyof T = DefaultExcludedFields<T>
>(this: SnapshotStore<T, K, Meta, ExcludedFields>, id: string, data: Partial<Snapshot<T, K, Meta, ExcludedFields>>): void {
  try {
    if (!this.dataStore) {
      throw new Error("Data store is not initialized");
    }
    ...
    let existingSnapshot: Snapshot<T, K, Meta, ExcludedFields> | undefined;
    if (this.dataStore.has(id)) {
      existingSnapshot = this.dataStore.get(id);
    }

    // Merge with existing data or create new snapshot
    const mergedSnapshot: Snapshot<T, K, Meta, ExcludedFields> = {
      ...(existingSnapshot || {} as Snapshot<T, K, Meta, ExcludedFields>),
      ...data,
      id: data.id || id,
...
>(): Promise<Snapshot<T, K, Meta, ExcludedFields>[] | undefined> {
  try {
    const items: Snapshot<T, K, Meta, ExcludedFields>[] = [];
