...
export const LifecycleMethods = {
  ...
  >(
    ...
    snapshots: Snapshot<T, K, Meta, ExcludedFields> | Snapshot<T, K, Meta, ExcludedFields>[],
    ...
    const createdSnapshots: Snapshot<T, K, Meta, ExcludedFields>[] = snapshotsArray.map(
      (snapshot) => {
        // Use createSnapshotInstance to build a full snapshot
        const completeSnapshot = createSnapshot<T, K, Meta, ExcludedFields>({
  ...
  >(
    this: SnapshotStore<T, K, Meta, ExcludedFields>,
    newState: Snapshot<T, K, Meta, ExcludedFields> & {
      isInitialized: true;
      initializedAt: Date;
      version?: string;
  ...
  getCurrentState: function <
    ...
    Meta = DefaultMeta<T, K>,
    ExcludedFields extends keyof T = DefaultExcludedFields<T>
  >(this: SnapshotStore<T, K, Meta, ExcludedFields>): Snapshot<T, K, Meta, ExcludedFields> | null {
  ...
  getStates: function <
    ...
    Meta = DefaultMeta<T, K>,
    ExcludedFields extends keyof T = DefaultExcludedFields<T>
  >(this: SnapshotStore<T, K, Meta, ExcludedFields>): Snapshot<T, K, Meta, ExcludedFields>[] {
    return this.states;
  },
  ...
  >(
    ...
    const snapshots = await this.getAllSnapshots?.(
      ...
      (snapshot: Snapshot<T, K, Meta, ExcludedFields>) => snapshot?.subscribers?.includes(subscriber)
    );
  ...
  getInitialState: function <
    ...
    ExcludedFields extends keyof T = DefaultExcludedFields<T>
  >(this: SnapshotStore<T, K, Meta, ExcludedFields>): Snapshot<T, K, Meta, ExcludedFields> {
    return {} as Snapshot<T, K, Meta, ExcludedFields>;
  },
  ...
  >(
    ...
    if (snapshot) {
      ...
      for (const [, store] of snapshotStores) {
        if (store.snapshots.some((s: Snapshot<T, K, Meta, ExcludedFields>) => s.id === snapshot.id)) {
          result.push(store);
        }
      }
    ...
    for (const store of snapshotStores.values()) {
      if (store.snapshots.some((s: Snapshot<T, K, Meta, ExcludedFields>) => s.id === id)) {
  ...
  >(
    ...
    if (snapshotId) {
      const existingSnapshot = this.snapshots.find((s: SnapshotUnion<T, K, Meta>) => {
        return isSnapshot<T, K, Meta, ExcludedFields>(s) && s.id === snapshotId;
      });
      ...
      if (existingSnapshot) {
        ...
        if (snapshot && isSnapshot<T, K, Meta, ExcludedFields>(snapshot)) {
  ...
  >(
    ...
    if (!(this as any).snapshotSubscribers) {
      (this as any).snapshotSubscribers = new Map<
        string,
        Array<(snap: Snapshot<T, K, Meta, ExcludedFields>) => void>
      >();
    }
