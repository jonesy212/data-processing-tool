...
import { convertEventsToRecord } from '@/app/components/typings/convertSnapshotEvents';
import { Snapshot } from "../types"; // adjust path to where Snapshot<T,K> lives
...
export const UtilMethods = {
  ...
  >(
    objA: Snapshot<T, K, Meta, ExcludedFields>,
    objB: Snapshot<T, K, Meta, ExcludedFields>
  ): boolean {
    ...
    for (let key of keysA) {
      if (objA[key as keyof Snapshot<T, K, Meta, ExcludedFields>] !== objB[key as keyof Snapshot<T, K, Meta, ExcludedFields>]) {
        return false;
      }
    }
  ...
  >(
    ...
    itemDifferences: Record<
      keyof Snapshot<T, K, Meta, ExcludedFields>,
      {
        snapshot1: any;
        snapshot2: any;
        differences: { [K in keyof Snapshot<T, K, Meta, ExcludedFields>]?: { value1: any; value2: any } };
      }
    >;
  } | null {
    if (!snap1 || !snap2) return null;
 ...
 getDataStoreMap: function <
  ...
  determineCategory: function <
    ...
    Meta extends DefaultMeta<T, K> = DefaultMeta<T, K>,
    ExcludedFields extends keyof T = DefaultExcludedFields<T>
  >(input: Snapshot<T, K, Meta, ExcludedFields> | SnapshotStore<T, K, Meta, ExcludedFields> | string): string {
