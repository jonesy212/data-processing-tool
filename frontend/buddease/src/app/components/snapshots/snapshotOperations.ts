...
// interface SnapshotOperations<T extends  BaseData<T>, K extends T = T, Meta extends StructuredMetadata<T, K> = StructuredMetadata<T, K>> {
//   // Existing methods
...
//   onSnapshots: (snapshots: Snapshots< BaseData<T>, Meta>) => void;
//   events: any[];
//   parentId: string;
//   childIds: string[];
...
//   getSnapshots: (category: string, data: Snapshots< BaseData<T>, Meta>) => Snapshots< BaseData<T>, Meta>;
//   getSnapshotItems: (category: symbol | string | Category | undefined, snapshots: SnapshotsArray< BaseData<T>, Meta>) => Promise<{ snapshots: SnapshotItem<T, K>[]; }>;
...
//       batchUpdateSnapshotsSuccess: (snapshots: Snapshots< BaseData<T>, Meta>) => void;
//       batchUpdateSnapshotsFailure: (snapshotManager: SnapshotManager<T, K>, snapshot: Snapshot<T, K>, payload: { error: Error }) => void;
//       batchUpdateSnapshotsRequest: (snapshots: Snapshots< BaseData<T>, Meta>) => void;
//       createSnapshots: (snapshots: Snapshots< BaseData<T>, Meta>) => void;
//       batchTakeSnapshot: (snapshot: Snapshot<T, K>) => void;
//       batchTakeSnapshotsRequest: (snapshots: Snapshots< BaseData<T>, Meta>) => void;
//       deleteSnapshot: (id: string) => void;
//       batchFetchSnapshots: (criteria: any) => Promise<Snapshots< BaseData<T>, Meta>>;
//       batchFetchSnapshotsSuccess: (snapshots: Snapshots< BaseData<T>, Meta>) => void;
//       batchFetchSnapshotsFailure: (snapshotManager: SnapshotManager<T, K>, snapshot: Snapshot<T, K>, payload: { error: Error }) => void;
//       filterSnapshotsByStatus: (status: string) => Snapshots< BaseData<T>, Meta>;
...
//       data:  BaseData<T>,
//       index: number
//     ) => SnapshotUnion< BaseData<T>, Meta>
//   ) => Promise<SnapshotsArray< BaseData<T>, Meta>>;
...
//   configureSnapshot: <T extends  BaseData<T>, K extends T = T, Meta extends StructuredMetadata<T, K> = StructuredMetadata<T, K>>(
//     id: string,
...
// const getParentId = (snapshot: Snapshot< BaseData<T>, BaseData>): string | null => {
//   return snapshot.parentId || null;
// };
// const getChildIds = (childSnapshot: Snapshot< BaseData<T>, BaseData>): string[] => {
...
//   category: Category | undefined,
//   categoryProperties: CategoryProperties | undefined,
//   snapshot: Snapshot< BaseData<T>, BaseData>,
//   timestamp: string | number | Date | undefined,
//   type: string,
//   event: Event,
//   id: number,
//   snapshotStore: SnapshotStore< BaseData<T>, BaseData>,
//   data:  BaseData<T>,
//   callback: (
//     storeIds: number[],
//     snapshotId: string,
...
//   ) => SnapshotUnion< BaseData<T>, Meta>
// ): Promise<SnapshotsArray< BaseData<T>, Meta>> => {
//   const snapshotsArray: SnapshotsArray< BaseData<T>, Meta> = [];
...
//     snapshotStore: SnapshotStore< BaseData<T>, BaseData>;
//     data: Data<T>;
//   }) => Snapshot< BaseData<T>, BaseData> // This provider generates Snapshot instances
// ): Promise<Snapshot< BaseData<T>, BaseData> | null> => {
...
// const handleSnapshot = <T extends  BaseData<T>, K extends T = T, Meta extends StructuredMetadata<T, K> = StructuredMetadata<T, K>>(
//   id: string,
//   snapshotId: string,
//   snapshot: T | null,
//   snapshotData:  BaseData<T>,
...
// const validateSnapshot = (snapshot: Snapshot< BaseData<T>, BaseData>): boolean => {
...
//   id: any; snapshot: Snapshot< BaseData<T>, BaseData>;
//   data: Data<T>;
// }> | undefined
// ): Promise<Snapshot< BaseData<T>, BaseData>> => {
...
//   snapshot: Snapshot< BaseData<T>, BaseData>,
//   subscribers: Subscriber< BaseData<T>, BaseData>[]
// ): Promise<{ snapshot: Snapshot< BaseData<T>, BaseData>; }> => {
...
//   return { snapshot };
// };
// const removeSnapshot = (snapshotToRemove: SnapshotStore< BaseData<T>, BaseData>): void => {
//   snapshotToRemove.clearSnapshot();
// };

// const updateSnapshot = (
//   snapshotId: string,
//   data: Map<string, Snapshot< BaseData<T>, BaseData>>,
//   events: Record<string, CalendarManagerStoreClass< BaseData<T>, BaseData>[]>,
//   snapshotStore: SnapshotStore< BaseData<T>, BaseData>,
//   dataItems: RealtimeDataItem[],
//   newData: Snapshot< BaseData<T>, BaseData>,
...
// const getSnapshots = (category: string, data: Snapshots< BaseData<T>, Meta>): Snapshots< BaseData<T>, Meta> => {
...
// ): Promise<{ snapshots: SnapshotItem< BaseData<T>, BaseData>[] }> => {
//   const snapshotItems: SnapshotItem< BaseData<T>, BaseData>[] = snapshots
//     .filter(snapshot => snapshot.category === category)
//     .map(snapshot => {
//       // Ensure content aligns with expected type in SnapshotItem
//       const content: Content< BaseData<T>, BaseData> | undefined =
//         typeof snapshot.content === 'string'
//           ? undefined // Or handle string content conversion if needed
//           : snapshot.content;
//       return {
//         ...snapshot,
//         content, // Now either undefined or Content< BaseData<T>, BaseData>
//       } as SnapshotItem< BaseData<T>, BaseData>; // Casting to ensure this is a SnapshotItem
//     });

//   return { snapshots: snapshotItems };
// };

// const getSnapshotContainer = <T extends  BaseData<T>, K extends T = T, Meta extends StructuredMetadata<T, K> = StructuredMetadata<T, K>>(
//   id: string | number,
//   snapshotFetcher: (id: string | number) => Promise<{
//     category: string;
//     timestamp: string;
...
//     getSnapshots: () => Snapshots< BaseData<T>, K>;
//     getAllSnapshots: () => Snapshots< BaseData<T>, K>;
//     generateId: () => string;
//     compareSnapshots: (snapshot1: Snapshot<T, K>, snapshot2: Snapshot<T, K>) => number;
//     compareSnapshotItems: (item1: any, item2: any) => number;
//     filterSnapshotsByCategory: (category: string) => Snapshots< BaseData<T>, Meta>;
//     filterSnapshotsByTag: (tag: string) => Snapshots< BaseData<T>, Meta>;
...
//   notify: () => void;
//   notifySubscribers: () => void;
//   getSnapshots: () => Snapshots< BaseData<T>, Meta>;
//   getAllSnapshots: () => Snapshots< BaseData<T>, Meta>;
//   generateId: () => string;
//   compareSnapshots: (snapshot1: Snapshot<T, K>, snapshot2: Snapshot<T, K>) => number;
//   compareSnapshotItems: (item1: any, item2: any) => number;
//   filterSnapshotsByCategory: (category: string) => Snapshots< BaseData<T>, Meta>;
...
// const configureSnapshot = <T extends  BaseData<T>, K extends T = T, Meta extends StructuredMetadata<T, K> = StructuredMetadata<T, K>>(
//   id: string,
//   snapshotData: SnapshotData<T, K>,
//   category?: string | symbol | Category,
//   callback?: ((snapshot: Snapshot<T, K>) => void) | undefined,
