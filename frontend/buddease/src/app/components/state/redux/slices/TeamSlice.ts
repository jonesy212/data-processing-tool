...
export const useTeamManagerSlice = createSlice({
  ...
  reducers: {
    ...
    removeTeam: (state, action: PayloadAction<string>) => {
      const teamIndex = state.teams.findIndex(team => team.teamName === action.payload);
      if (teamIndex !== -1) {
    ...
    updateTeamDetails: (state, action: PayloadAction<TeamData>) => {
      const { teamName } = action.payload;
    ...
    setTeamLeader: (state, action: PayloadAction<string>) => {
      ...
      updateTeamProjectList: (state, action: PayloadAction<{ teamName: string; projects: WritableDraft<Project[]> }>) => {
    ...
    updateTeamCollaborationTools: (state, action: PayloadAction<{
      ...
      updateTeamGlobalCollaboration: (state, action: PayloadAction<{ teamName: string; isGlobal: boolean }>) => {
