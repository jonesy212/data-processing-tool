...
export abstract class BaseDatabaseService implements DatabaseService {
  ...
  async findOne(identifier: string): Promise<any> {
    try {
      ...
      const result = await this.client.query(`SELECT * FROM your_table WHERE id = $1`, [identifier]);
  ...
  async update(data: any): Promise<any> {
    try {
      ...
      const result = await this.client.query(
        `UPDATE your_table SET ${Object.keys(updatedFields).map((key, index) => `${key} = $${index + 1}`).join(', ')} WHERE id = $${Object.keys(updatedFields).length + 1}`,
  ...
  async create(data: any): Promise<any> {
    try {
      ...
      const result = await this.client.query(
        `INSERT INTO your_table (${Object.keys(data).join(', ')}) VALUES (${Object.keys(data).map((_, index) => `$${index + 1}`).join(', ')}) RETURNING *`,
        Object.values(data)
      );
  ...
  async delete(identifier: string): Promise<any> {
    try {
      ...
      const result = await this.client.query(`DELETE FROM your_table WHERE id = $1`, [identifier]);
  ...
  async bulkCreate(data: any[]): Promise<any[]> {
    try {
      ...
      const result = await this.client.query(`INSERT INTO your_table (${Object.keys(data[0]).join(', ')}) VALUES ${values} RETURNING *`);
  ...
  async batchInsert(data: any[]): Promise<any[]> {
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        const placeholders = new Array(Object.keys(rowData).length).fill('$').map((v, index) => v + (index + 1)).join(', ');
