...
import { FixPlan, RelationshipMap, TSCompilerError, ConfidenceFactors } from '@/app/error-analyzer/index';
import { FixStrategy } from '@/app/error-analyzer/index';
  ...
  private calculateContextClarityScore(error: TSCompilerError, relationshipMap: RelationshipMap): number {
    ...
    if (identifier) {
      ...
      if (usages && usages.length > 0) {
        ...
        const hasTypeContext = usages.some(usage => 
          usage.context.includes(':') || usage.context.includes('interface')
        );
        if (hasTypeContext) {
          score += 3;
        }
