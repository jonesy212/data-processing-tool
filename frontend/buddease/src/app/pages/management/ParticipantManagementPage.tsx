...
const ParticipantManagementPage = () => {
  ...
  useEffect(() => {
    // Dispatch action to fetch participant data when the component mounts
      dispatch(ToolbarActions.fetchParticipantData({ participantData: {} as ParticipantData }));
  }, [dispatch, userId]);

  const handleAddParticipant = () => {
    // Dispatch action to add a new participant
    dispatch(ToolbarActions.addParticipant({ userId, participant: { name: 'New Participant', role: 'Member' } }));
  };
      ...
      <ul>
        {participantData.map(participant => (
          <li key={participant.id}>
            ...
            <button onClick={() => handleRemoveParticipant(participant.id)}>Remove</button>
          </li>
        ))}
      </ul>
    </div>
