...
import { useStores } from "@/app/state/context/AppContext";
...
const getAppContext = (userQuery: string): string | null => {
  ...
  if (userQuery.includes("phases")) return "phases";
  if (userQuery.includes("crypto")) return "crypto";
  if (userQuery.includes("project")) return "project";
  return null;
};
...
const generateAppResponse = (context: string | null): string => {
  switch (context) {
    ...
    case "crypto":
      return "Crypto features allow you to manage cryptocurrency-related functionality within the app.";
    case "project":
      return "Project management tools help you organize and track your work efficiently.";
    default:
