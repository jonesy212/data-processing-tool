...
export const setThreshold = <T extends CryptoData>(
  ...
  for (const data of parsedData) {
    ...
    if ('priceDisparity' in data.data) {
      ...
      if (typeof data.data.priceDisparity === 'number' && data.data.priceDisparity > threshold) {
        // If the disparity exceeds the threshold, log the potential arbitrage opportunity
        console.log(`Potential arbitrage opportunity detected for ${data.data.cryptocurrencyPair}`);
        ...
        console.log(`No significant price disparity detected for ${data.data.cryptocurrencyPair}`);
      }
    } else if (data.data.prices?.length && data.data.prices.length > 1) { // Check if prices array exists and has length greater than 1
