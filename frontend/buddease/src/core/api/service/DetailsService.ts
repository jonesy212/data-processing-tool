...
export const detailsApiService = {
  ...
  deleteDetailsItems: async (
    detailsItemIds: string[],
    options?: ApiRequestOptions<ClientNotificationMessages>
  ): Promise<void> => {
    try {
      await internalApiService.delete(`${API_BASE_URL}`, {
        ...options, // Spread any provided options
        config: {
          ...options?.config, // Merge with existing config
          data: { detailsItemIds }, // Add data to config
        }
      });
      notify({
