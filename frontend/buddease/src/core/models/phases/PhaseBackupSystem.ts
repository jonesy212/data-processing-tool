...
export class PhaseBackupSystem implements PhaseBackupSystem {
  ...
  async restoreBackup(backupId: string): Promise<void> {
    ...
    fs.writeFileSync(parsed.filePath, parsed.content, 'utf8');

    this.updateBackupRecordStatus(backupId, 'rolled-back');

    console.log(`♻️ Restored file from backup: ${parsed.filePath}`);
  }
...
public listRestorePoints(): RestorePoint[] {
  return this.getRestorePoints();
}
