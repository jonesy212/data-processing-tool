...
> {
  ...
  constructor(projectPath: string, globalState?: any,  major: number = 1, minor: number = 0, patch: number = 0) {
    ...
    // Private hash calculation method
  private calculateHash(data: any): string {
    // Implement your hash calculation logic
    const str = JSON.stringify(data);
    // Simple hash for demonstration - replace with actual hash function
    let hash = 0;
 ...
 // Getter to access the private structureHash
  ...
  public async getStructureHashAsync(): Promise<string | undefined> {
    return this.#structureHash;
  }
