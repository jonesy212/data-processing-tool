...
import { SnapshotMethodsImplementation } from '@/core/snapshots/methods/snapshotMethods';
import { ValidationMethods } from '@/core/snapshots/methods/validationMethods';
...
import { CommonDataStoreMethods, DataStore, EventRecord, InitializedState } from '@/core/state/stores/DataStore';
...
import { searchAPI } from "@/core/api/ApiSearch";
...
import { transformSubscriberAdvanced, transformSubscriberMappedAdvanced } from '@/core/snapshots/methods/advancedTransform';
import { BatchMethods } from '@/core/snapshots/methods/batchMethods';
import * as DataMethods from '@/core/snapshots/methods/dataMethods';
import * as FetchMethods from '@/core/snapshots/methods/fetchMethods';
import { LifecycleMethods } from '@/core/snapshots/methods/lifecycleMethods';
import { MapMethods } from "@/core/snapshots/methods/mappingMethods";
import { SubscriptionMethods } from '@/core/snapshots/methods/subscriptionMethods';
import { TransformMethods } from '@/core/snapshots/methods/transformMethods';
import * as VersionMethods from '@/core/snapshots/methods/versionMethods';
...
protected deletedSnapshots: Map<string, Snapshot<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields>> = new Map();
private delegate: Array<SnapshotStoreConfig<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields>> = [];
  ...
  processSnapshotData = async (
    ...
    try {
      // Use type assertion for payload since returnType might not exist
      const payloadWithReturn = payload as { returnType?: string };
      
      // Determine what to return based on input parameters
      if (payloadWithReturn?.returnType === 'map') {
        ...
        return snapshotsMap as SnapshotDataType<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields>;
      }

      if (payloadWithReturn?.returnType === 'store') {
        return snapshotStore as SnapshotDataType<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields>;
      }

      if (payloadWithReturn?.returnType === 'promise') {
        // Cast the promise to the expected type
        return (Promise.resolve({ snapshot: newData }) as unknown) as SnapshotDataType<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields>;
      }

      if (payloadWithReturn?.returnType === 'snapshotData') {
        ...
        const snapshotData: SnapshotData<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields> = {
          ...
          transform: (snap: any) => this.transformSnapshotMethod(snap),
          id: newData.id?.toString() || "",
          storeId: this.storeId,
          category: this.category?.toString() || "",
          serialize: () => JSON.stringify(newData),
          get: (key: string) => {
            return (newData as any)[key];
          },
          ...
          shared: this.topic || "",
          operations: this.getSnapshotOperations(),
          base: this.getBaseEntity(),
      ...
      // Default: return the newData as fallback
      return newData as SnapshotDataType<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields>;

    } catch (error) {
      console.error('Error in processSnapshotData:', error);
      // Return a valid default instead of undefined
      return new Map() as SnapshotDataType<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields>;
    }
  }
 ...
 */
  hasCachedData(id: string): boolean {
    return this._snapshotDataCache.has(id);
  }
...
handleActions(action: any): void {
  ...
  validate(options?: any, config?: any, action?: any): boolean {
    // Validate constructor params if provided
    if (options !== undefined && config !== undefined) {
    ...
    const isValid = this.validateInternalState();
  ...
  protected notifySubscribers(event: {
    type: 'delete';
    snapshotId: string;
    permanent: boolean;
  }): void {
...
public async getSnapshotConfig(snapshotId: string | null): Promise<SnapshotConfig<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields> | null> {
  try {
  ...
  async getSnapshotId(data: SnapshotData<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields>): Promise<string> {
      if (this.delegate && this.delegate.length > 0) {
        for (const delegateConfig of this.delegate) {
          if (delegateConfig && typeof delegateConfig.getSnapshotId === "function") {
            const id = await delegateConfig.getSnapshotId(data);
            if (id) {
              return id; // Always returns a string
            }
          }
        }
        // If no valid ID found, throw an error (better than returning undefined)
        throw new Error("Unable to generate snapshot ID - no valid delegate found");
      } else {
        throw new Error("Delegate is undefined or empty");
      }
  }
  ...
  private _storeOptions: SnapshotStoreOptions<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields> | undefined;
  
  // Public getter
  public get storeOptions(): SnapshotStoreOptions<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields> | undefined {
...
private transformSnapshot<U extends Data<U>, T extends BaseDataEntity>(
  ...
  setSnapshotData(params: SnapshotDataParams<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields>): 
    Map<string, Snapshot<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields>> {
    ...
    if (this.config && configUpdate) {
      this.config = this.config.map((configItem) => ({
        ...configItem,
        ...configUpdate,
        initialState: configUpdate.initialState ?? configItem.initialState ?? null,
