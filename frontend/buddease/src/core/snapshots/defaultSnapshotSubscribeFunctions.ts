...
>(
  ...
  // Ensure `snapshot` is of type `Snapshot<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields>`
  callback(snapshot);
...
): Promise<CoreSnapshot<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields>> => {
  try {
    ...
    const coreSnapshot: CoreSnapshot<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields> = {
