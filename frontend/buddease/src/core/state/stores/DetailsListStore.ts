...
import { CommunicationActionTypes } from "@/core/actions/CommunicationActions";
...
import { data, SnapshotConfig, SnapshotDataType } from "@/core/snapshots/SnapshotWithCriteria";
...
import { createSnapshotStoreOptions } from "@/core/typings/YourSpecificSnapshotType";
...
> implements DetailsListStore<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields>
{
  ...
  private createDefaultPhase(): Phase<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields> {
    return {
      ...
      date: new Date(),
      createdBy: ""
      // Initialize any other required properties of Phase here, based on Phase<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields> structure
  ...
  private async initSnapshotStore(
    ...
    const snapshotConfig: SnapshotConfig<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields> = {
      ...
      this.snapshotStore = new SnapshotStore<T, K, Meta, AttachmentType, ExcludedFields, IncludedFields>({storeId, name, version, schema, options, category, config, operation});
    }
