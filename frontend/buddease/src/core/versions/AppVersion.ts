...
> implements AppVersion {
  ...
  private async getBackendStructure(): Promise<BackendStructure> {
    ...
    const structureHash = backendStructure.structureHash;

    // Return a properly typed object that matches the expected interface
    return Promise.resolve({
      // Public properties accessible via getters
      structureHash: structureHash,
      version: "1.0.0",
      services: ["UserService", "AuthService"],
      databaseSchema: "v1.2",
      
      // Version info from this instance
      major: this.major,
      minor: this.minor,
      patch: this.patch,
      
      // Methods from BackendStructure
      toSecureMetadata: () => backendStructure.toSecureMetadata(),
      sanitize: (role?: UserRole, admin?: boolean) => backendStructure.sanitize(role, admin),
      getDatabaseSchema: () => backendStructure.getDatabaseSchema(),
      getVersion: () => backendStructure.getVersion(),
      getServices: () => backendStructure.getServices(),
      validateStructure: () => backendStructure.validateStructure(),
      serializeStructure: () => backendStructure.serializeStructure(),
      compareStructures: (other: BackendStructure) => backendStructure.compareStructures(other),
      migrateStructure: async (newStructure: BackendStructure) => backendStructure.migrateStructure(newStructure),
      
      // Public property
      globalState: backendStructure.globalState,
      
      // If you need setters
      setDatabaseSchema: (schema: DatabaseSchema) => { /* implementation */ },
      setServices: (services: ServiceSchema) => { /* implementation */ },
      
      // Structure access methods (add these to BackendStructure if needed)
      getStructure: () => backendStructure.getStructure?.(),
      getStructureAsArray: () => backendStructure.getStructureAsArray?.(),
      traverseDirectoryPublic: () => backendStructure.traverseDirectoryPublic?.(),
      
      // Hash methods
      getStructureHash: () => backendStructure.structureHash,
      setStructureHash: (hash: string) => backendStructure.setStructureHash(hash),
      updateStructureHash: async (newStructure?: any) => backendStructure.updateStructureHash(newStructure),
      
      // Other methods
      getStructureHashAndUpdateIfNeeded: async () => {
        if (!backendStructure.structureHash) {
          return await backendStructure.updateStructureHash();
        }
        return backendStructure.structureHash;
      },
      backendVersions: []
    } as unknown as BackendStructure); // Cast to BackendStructure if needed
  }
