...
const resetAppState = async () => {
  ...
  if (isUserLoggedInLocal) {
    ...
    if (isUsingRedux) {
      ...
      settingsStore.setUserData(userData); // Use MobX to set user data
    }
    ...
    if (isUsingRedux) {
      ...
      const settingsStore = useSettingManagerStore();
      settingsStore.resetUserPreferences(); // MobX action to reset user preferences
    }
    ...
    if (isUsingRedux) {
      ...
      const settingsStore = useSettingManagerStore();
      settingsStore.clearGuestData(); // MobX action to clear guest data
    }
